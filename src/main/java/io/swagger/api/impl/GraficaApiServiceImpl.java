...
public class GraficaApiServiceImpl extends GraficaApiService {
    ...
    static GraficaApiServiceImpl graficaApiService = new GraficaApiServiceImpl();
    ...
    public Response obtenerGrafica( @NotNull String usuarioId, SecurityContext securityContext) throws NotFoundException {
        ...
        Mediciones medicionesResponse = new Mediciones();

        for (int i = 0; i < mediciones.size(); i++) {

            if (mediciones.get(i).getUsuarioId().equalsIgnoreCase(usuarioId)){

                medicionesResponse.add(mediciones.get(i));
        ...
        String jsonInString = gson.toJson(medicionesResponse);

        if (medicionesResponse.size()!=0){
    ...
    public static void main(String[] args) {
        ...
        try {
            Response response = graficaApiService.obtenerGrafica("U1",securityContext);
